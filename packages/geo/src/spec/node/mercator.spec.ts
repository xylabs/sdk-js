...
import type {
  boundingBoxToBoundary as _boundingBoxToBoundary,
  boundingBoxToCenter as _boundingBoxToCenter,
  boundingBoxToPolygon as _boundingBoxToPolygon,
  MercatorBoundary, MercatorBoundingBox, MercatorLngLat, MercatorTile,
  tilesFromBoundingBox as _tilesFromBoundingBox,
  tileToBoundingBox as _tileToBoundingBox,
  tileToGeoJson as _tileToGeoJson,
  tileToPoint as _tileToPoint,
...
class MockLngLatBounds {
  ...
  getCenter() {
    return new MockLngLat((this._sw.lng + this._ne.lng) / 2, (this._sw.lat + this._ne.lat) / 2)
  }
...
describe('Mercator tile functions', () => {
  ...
  describe('tileToBoundingBox', () => {
    let tileToBoundingBox: typeof _tileToBoundingBox
  ...
  describe('tileToPoint', () => {
    let tileToPoint: typeof _tileToPoint
  ...
  describe('tileToGeoJson', () => {
    let tileToGeoJson: typeof _tileToGeoJson
  ...
  describe('boundingBoxToCenter', () => {
    let boundingBoxToCenter: typeof _boundingBoxToCenter
    ...
    it('returns center of a symmetric bounding box', () => {
      ...
      const center = boundingBoxToCenter(box as unknown as MercatorBoundingBox)
    ...
    it('respects decimal precision parameter', () => {
      const box = new MockLngLatBounds(
        new MockLngLat(-10.123_456_789, -20.987_654_321),
        new MockLngLat(10.123_456_789, 20.987_654_321),
      )
      const center2 = boundingBoxToCenter(box as unknown as MercatorBoundingBox, 2)
  ...
  describe('boundingBoxToBoundary', () => {
    let boundingBoxToBoundary: typeof _boundingBoxToBoundary
    ...
    it('returns 5 points (closed boundary)', () => {
      ...
      const boundary = boundingBoxToBoundary(box as unknown as MercatorBoundingBox)
    ...
    it('returns NW, NE, SE, SW, NW corners in order', () => {
      ...
      const boundary: MercatorBoundary = boundingBoxToBoundary(box as unknown as MercatorBoundingBox) as MercatorBoundary
      // NW
  ...
  describe('boundingBoxToPolygon', () => {
    let boundingBoxToPolygon: typeof _boundingBoxToPolygon
    ...
    it('returns a Polygon type', () => {
      ...
      const polygon = boundingBoxToPolygon(box as unknown as MercatorBoundingBox)
  ...
  describe('tilesFromBoundingBox', () => {
    let tilesFromBoundingBox: typeof _tilesFromBoundingBox
    ...
    it('returns tiles covering a small area', () => {
      ...
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, 10)
    ...
    it('all returned tiles have the requested zoom level', () => {
      ...
      const zoom = 5
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, zoom)
      for (const tile of tiles) {
    ...
    it('returns all tiles at low zoom (< 4)', () => {
      ...
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, 2)
    ...
    it('zoom 0 returns 1 tile', () => {
      ...
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, 0)
    ...
    it('zoom 1 returns 4 tiles', () => {
      ...
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, 1)
    ...
    it('zoom 3 returns 64 tiles (8x8)', () => {
      ...
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, 3)
    ...
    it('larger area produces more tiles than smaller area at same zoom', () => {
      ...
      const zoom = 6
      const smallTiles = tilesFromBoundingBox(smallBox as unknown as MercatorBoundingBox, zoom)
      const largeTiles = tilesFromBoundingBox(largeBox as unknown as MercatorBoundingBox, zoom)
    ...
    it('no duplicate tiles in result', () => {
      ...
      const tiles = tilesFromBoundingBox(box as unknown as MercatorBoundingBox, 5)
