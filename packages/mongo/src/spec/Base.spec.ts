...
describe('BaseMongoSdk', () => {
  ...
  describe('CRUD operations', () => {
    ...
    beforeEach(() => {
      vi.spyOn(MongoClientWrapper, 'get').mockReturnValue(mockWrapper as any)
      vi.clearAllMocks()
      vi.spyOn(MongoClientWrapper, 'get').mockReturnValue(mockWrapper as any)
    })
    ...
    it('updateOne delegates to collection with upsert false', async () => {
      await sdk.updateOne({ name: 'test' }, { $set: { value: 1 } } as any)
    ...
    it('upsertOne delegates to collection with upsert true', async () => {
      await sdk.upsertOne({ name: 'test' }, { $set: { value: 1 } } as any)
      expect(mockCollection.updateOne).toHaveBeenCalledWith(
        { name: 'test' },
        { $set: { value: 1 } },
        { upsert: true },
    ...
    it('replaceOne delegates to collection', async () => {
      const item = { name: 'replaced' } as any
    ...
    it('useMongo connects and disconnects', async () => {
      const result = await sdk.useMongo(async (client) => {
    ...
    it('useMongo disconnects even on error', async () => {
      await expect(sdk.useMongo(async () => {
    ...
    it('useMongo throws when connection fails', async () => {
      ...
      await expect(sdk.useMongo(async () => 'result')).rejects.toThrow('Connection failed')
    })
  })
})
