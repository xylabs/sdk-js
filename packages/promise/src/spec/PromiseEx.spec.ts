...
describe('PromiseEx', () => {
  it('resolves like a normal promise', async () => {
    const p = new PromiseEx<string>((resolve) => { resolve?.('hello') })
    expect(await p).toBe('hello')
  })

  it('stores a value', async () => {
    const p = new PromiseEx<string, number>((resolve) => { resolve?.('done') }, 42)
  ...
  it('sets cancelled when value callback returns true', async () => {
    const p = new PromiseEx<string, number>((resolve) => { resolve?.('done') }, 10)
    p.value(() => true)
  ...
  it('sets cancelled via then with onvalue returning true', async () => {
    const p = new PromiseEx<string, number>((resolve) => { resolve?.('ok') }, 5)
